    ...
    for lpath_original in tqdm(lo1a_fnames, leave=False):
        ...
        for bi in tqdm(range(n_banks)):
            ...
            try:
                ...
                vegas_n_rows4 = len(session.VEGAS_SPURCHAN)
                ...
                for i in tqdm(range(vegas_n_rows6), leave=False):
                    ...
                    for si in range(len(vegas_new_data4['SPURCHAN'])):
                        vegas_new_data4['SPURCHAN'][si] += channel_shift_i 
                        if vegas_new_data4['SPURCHAN'][si] < 0:
                            vegas_new_data4['SPURCHAN'][si] = np.nan
                ...
                write_new_VEGAS(session.VEGAS_FNAME, vpath_modified, vegas_new_hdr0, vegas_new_data4, vegas_n_rows4,vegas_new_data6, vegas_n_rows6)

                # Write a new LO1A FITS file
                write_new_LO1A(lpath_original, lpath_modified, lo1a_new_hdr0, lo1a_new_data3, lo1a_n_rows3)
            except:
                print(f"Sorry, something went wrong with {vpath_modified}")